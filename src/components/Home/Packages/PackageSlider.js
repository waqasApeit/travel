...
export default function PackageSlider({ packages }) {
  const ShowPackagePrice = (priceDetails) => {
    if (!priceDetails || priceDetails.length === 0) return null;
    const filteredPrices = priceDetails.filter(
      (p) => p.type?.toLowerCase() !== "without-bed"
    );
    if (filteredPrices.length === 0) return null;
    const sortedPrices = filteredPrices.sort(
      (a, b) => Number(a.sale_per_person) - Number(b.sale_per_person)
    );
      ...
      >
        {packages.map((item, index) => (
          <SwiperSlide id={index}>
            <div className="home-package-new-card">
              <div className="home-package-new-card-header">
                <div className="p-heading-top">
                  <h4>{item.title}</h4>
                  <h5>
                    {item?.currency_code}{" "}
                    {ShowPackagePrice(item?.price_Details)}{" "}
                    <span>
                      /
              ...
              <div className="home-package-new-card-body">
                <ul className="p-0 m-0">
                  <li>
                    <LuCircleCheck size={35} />
                    {item?.flights.length !== 0 && item?.visas.length !== 0
                      ? "Flights and Visa Included"
                      : item?.flights.length !== 0
                      ? "Flights Included"
                      : item?.visas.length !== 0
                      ? "Visa Included"
                      : "Flights and Visa Not Included"}
                  </li>
                  <li>
                    <LuCircleCheck size={35} />
                  ...
                  {item.hotels.map((hotel, hotelIndex) => (
                    <li key={hotelIndex}>
                      <LuCircleCheck size={35} />{" "}
                      {moment(hotel.hotel.booked_to).diff(
                        moment(hotel.hotel.booked_from),
                        "days"
                      )}{" "}
                      Nights in {hotel.hotel.city}
                    </li>
                  ))}
                  <li>
                    <LuCircleCheck size={35} /> {item.star_rating} star package
                  </li>
                </ul>
                <Link href={`/packages/${item.slug}`} className="w-100">
                  <button className="btn home-package-button">
                    Purchase Now
                  </button>
                </Link>
              </div>
        ...
        <div className="row">
            ...
            <div className={`card border-0 ${styles.umrahCard}`}>
              ...
              <div className="position-relative">
                <Image
                  ...
                  height="100"
                  width="600"
                  style={{ height: "17em", objectFit: "cover" }}
                />
              ...
              <div className={`card-body ${styles.cardBody}`}>
                <h4 className="mb-3">Regular Umrah</h4>
                {/* 
          ...
          <hr /> */}
                ...
                <ul className={`list-unstyled ${styles.featureList}`}>
                  <li>
                    <FaPassport /> Include Passport & Visa
                  </li>
                  <li>
                    <FaHotel /> Mecca - Al Safwah Royale Hotel
                  </li>
                  <li>
                    <FaHotel /> Medina - Rawdat Al Safwah Hotel
                  </li>
                  <li>
                    <FaSuitcase /> Free 30kg Suitcase
                  </li>
                  <li>
                    <FaPlane /> Premium Economy Seat
                  </li>
                  <li>
                    <FaUtensils /> 3 Time Meals
                  </li>
                </ul>

                {/* Price */}
                <div className={`text-center ${styles.priceBox}`}>
                  <span className="text-muted small d-block">Start From</span>
                  <h4 className="mb-0">1600 USD</h4>
                </div>

                {/* Button */}
                <button className={`btn w-100 mt-3 ${styles.detailsBtn}`}>
                  Package Detail
                </button>
              </div>
            </div>
