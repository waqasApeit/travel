    ...
    <div className="container hotel-card">
      {hotels.length === 0 && isLoading === false && (
        <div className="alert alert-warning text-center" role="alert">
          ...
          <Link href='/'>
            <button className='btn btn-success mt-2'><FaHome size={15} /> Go to Home</button>
          </Link>
        </div>
      )}
                  ...
                  <Image
                    src={
                      imgFallbacks[item?.id]
                        ...
                        : normalizeImageSrc(hotelDetails[item?.id]?.images || '/images/hotelloadimg.jpg')
                    }
                    width={400}
                    height={250}
                  ...
                  <div className="card-body">
                    <div className="d-flex justify-content-between align-items-center flex-wrap">
                      <div>
                        ...
                        {item?.metadata?.stars ? (
                          !isNaN(item.metadata.stars) ? (
                            // ⭐ Numeric rating (4, 3.5, etc.)
                            <>
                              {Array(Math.floor(Number(item.metadata.stars)))
                                .fill(0)
                                .map((_, i) => (
                              ...
                              {/* Half star support */}
                              {Number(item.metadata.stars) % 1 !== 0 && (
                                <FaStarHalfAlt className="text-warning me-1" />
                              )}
                            </>
                          ) : (
                            // 🏨 Text rating (4lux, Deluxe, etc.)
                      ...
                      <span className="badge bg-secondary p-2 mt-2 mt-md-0">
                        {moment(check_in).format('ll')} <FaArrowRightLong /> {moment(check_out).format('ll')}
                      </span>
                    </div>
                    ...
                    {hotelDetails[item?.id] ? (
                      <p className="card-text small text-success mb-2">
                        ...
                        {hotelDetails[item?.id]?.address || 'Address not available'}
                      </p>
                    ) : (
                    ...
                    {hotelDetails[item?.id] ? (
                      <div className="d-flex flex-wrap mb-2 justify-content-start">
                        {hotelDetails[item?.id]?.facilities.slice(0, 7).map((amenity, i) => (
                          <span
                    ...
                    <div className="d-flex justify-content-between align-items-center flex-wrap">
                      <div className="mb-2 mb-md-0">
                        <small className="fw-bold d-block">
                          <PriceDisplay price={(item.metadata.min_price / daysDiff)} currency={item?.metadata.currency} /> / Night
                        </small>
                        <small
                      ...
                      <div className='text-center'>
                        <p className="fw-bold mb-0">
                          ...
                          {item?.currency} {item?.price}<br/>
                          <span style={{ fontSize: "11px" , fontWeight :'normal' }} className='small'>Vat and Taxes included</span>
                        </p>
                        <Link target='_blank' onClick={() => SetRoomsData(item.id)} href={`/hotels/${makingSlug(item.name)}?id=${item.id}&code=${ProviderShortNames(item.provider)}`}>
                          <button className="btn btn-success">View Detail <LiaAngleRightSolid /></button>
                        </Link>
                      </div>
                    </div>
                  </div>
