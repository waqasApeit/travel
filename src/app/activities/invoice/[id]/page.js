...
export default function Page() {
  ...
  function convertToCustomerCurrency(price) {
    ...
    // If currency_code matches customer_currency, no conversion needed
                    ...
                    <table className={`${styles.invoicetable} ${styles.tablestripe3} w-100`}>
                      ...
                      <tbody>
                        <tr className="text-center">
                          ...
                          <td>{voucherDetail?.adults} * {voucherDetail?.customer_currency} {convertToCustomerCurrency(voucherDetail?.activity?.sale_price)}</td>
                          <td>{voucherDetail?.customer_currency} {convertToCustomerCurrency(voucherDetail?.adult_price)}</td>
                        </tr>
                        {voucherDetail?.children > 0 && (
                          <tr className="text-center">
                            <td>Child{voucherDetail?.children !== 1 ? 'ren' : ''}</td>
                            <td>{voucherDetail?.children} * {voucherDetail?.customer_currency} {convertToCustomerCurrency(voucherDetail?.activity?.child_sale_price)}</td>
                            <td>{voucherDetail?.customer_currency} {convertToCustomerCurrency(voucherDetail?.child_price)}</td>
                          </tr>
                        )}
                        ...
                        <tbody>
                          {voucherDetail?.additional_services.map((item, index) => (
                            <tr className="text-center" key={index}>
                              <td>{item.name} </td>
                              <td>{item.quantity} * {voucherDetail?.customer_currency} {convertToCustomerCurrency(item?.price)}</td>
                              <td>{voucherDetail?.customer_currency} {convertToCustomerCurrency(item?.total)}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
                  ...
                  <div className="row justify-content-between">
                    <div className="col-auto mb-3">
                      <div className="invoice-left">
                        ...
                        <p className="mb-0">The invoice displays the complete pricing details of your activity <br /> booking, inclusive of all applicable taxes.<br />
                          Please verify the information and retain this invoice for your records.</p>
                      </div>
                    </div>
                    <div className="col-auto">
                      <table className={`${styles.totaltable} w-100`}>
                        <tr>
                          ...
                          <td>{voucherDetail?.customer_currency} {voucherDetail?.customer_total}</td>
                        </tr>
                        <tr>
