    ...
    <div>
      <Grid grow gutter="xs">
        <GridCol span={{ base: 12, md: 12, lg: 6 }} className="position-relative">
          <Image height={310} className="w-100 object-fit-cover rounded" alt='Hotel Image' width={400} src={imageList[0]} />
          ...
          <Grid grow gutter="xs">
            {imageList.slice(0, 6).map((ListItem, index) => (
              <GridCol key={index} span={{ base: 4, md: 4, lg: 4 }}>
                <Image height={150} width={200} alt={`Hotel image ${index + 1}`} className="w-100 object-fit-cover rounded" src={ListItem} />
              </GridCol>
            ))}
          </Grid>
        </GridCol>
